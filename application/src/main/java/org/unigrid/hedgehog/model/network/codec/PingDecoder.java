 ...
 */
	...
	public final class PingDecoder
			extends AbstractReplayingDecoder<Ping>
			implements PacketDecoder<Ping> {
	
		@Override
		public Optional<Ping> typedDecode(ChannelHandlerContext ctx, ByteBuf in) {
			Ping ping = new Ping();
			ping.setNanoTime(in.readLong());
			ping.setResponse((in.readByte() & 0x01) == 1);
			in.skipBytes(7);
